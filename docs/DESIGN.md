...
本システムは、ローカルGPU上で動作するAIモデル（Microsoft Foundry Local）とクラウドベースのAIエージェント（Azure AI Agent Framework）を組み合わせた**ハイブリッド型医療トリアージ支援システム**です。

### 1.1 目的

- 非緊急医療トリアージのガイダンスを提供する
...
| パッケージ | バージョン | 用途 |
|-----------|-----------|------|
| `agent-framework-azure-ai` | プレリリース | Azure AIエージェント |
| `azure-identity` | >=1.17.1 | Azure認証 |
...
| パラメータ | 値 | 説明 |
|-----------|-----|------|
| `FOUNDRY_LOCAL_BASE` | `http://127.0.0.1:52403` | Foundry Localエンドポイント |
| `FOUNDRY_LOCAL_MODEL_ID` | `Phi-4-mini-instruct-cuda-gpu:5` | 使用モデル |
| `max_tokens` | 256 | ローカルモデル最大トークン |
...
| コードフェンス混入 | `_strip_code_fences()`で自動除去 |
...
```powershell
python hybrid-ai-agent.py
```

### 6.3 出力例

```
[ローカルツール] POST http://127.0.0.1:52403/v1/chat/completions
[ローカルツール] モデルからの生コンテンツ:
{"overall_assessment": "...", "notable_abnormal_results": [...]}
...
| 拡張項目 | 内容 |
|----------|------|
| 複数ツール追加 | 画像診断、薬剤相互作用チェック等 |
| モデル切り替え | 症状に応じた専門モデルの動的選択 |
| 多言語対応 | 英語・中国語等のプロンプト追加 |
| ログ・監査 | 処理履歴の永続化 |

### 7.2 ハイブリッドアーキテクチャの利点

1. **レイテンシ最適化**: 頻繁な処理はローカルで実行
2. **コスト削減**: クラウドAPI呼び出し回数の削減
3. **オフライン対応**: ローカルモデルのみでの限定稼働が可能
4. **プライバシー**: 機密データのローカル処理
...
├── hybrid-ai-agent.py    # メインアプリケーション
├── requirements.txt       # 依存パッケージ
├── work-log.md           # 開発ログ
├── DESIGN.md             # 本設計書
└── image.png             # NPUリソース使用状況スクリーンショット
```
